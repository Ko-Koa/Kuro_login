...
from .utils import get_current_timestamp, get_guid, geetest_m
...
class Geetest:
    ...
    async def _send_load(self):
        ...
        try:
            response = await self.client.get(url="https://gcaptcha4.geetest.com/load", params=params)
    ...
    async def _fetch_track(self, pow_message: str, sign: str):
        ...
        if captcha_type == "slide":
            return await slide.get_track(geetest_info=self.geetest_info, message=pow_message, sign=sign,
                                         client=self.client)
    ...
    async def _verify(self, w: str):
        ...
        try:
            response = await self.client.get(url="https://gcaptcha4.geetest.com/verify", params=params)
            response.raise_for_status()
        except httpx.HTTPError as e:
